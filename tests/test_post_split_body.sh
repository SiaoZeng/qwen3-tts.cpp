...
LOG_FILE="${QWEN3_TTS_TEST_LOG:-/tmp/qwen3-tts-post-split.log}"
BODY='{"text":"你好","language":"zh","voice":"lina"}'
...
rm -f "$LOG_FILE" /tmp/qwen3-tts-post-split.response
...

{
    ...
    sleep 1
    printf '%s' "$BODY"
} | socat - "TCP:127.0.0.1:${PORT}" >/tmp/qwen3-tts-post-split.response || true

wait_for_idle || {
    printf 'split POST request did not return to idle\n' >&2
    cat "$LOG_FILE" >&2
    exit 1
}

grep -q '\[TTS\] "你好" lang=zh voice=lina' "$LOG_FILE"

printf 'split POST body recovered cleanly\n'
